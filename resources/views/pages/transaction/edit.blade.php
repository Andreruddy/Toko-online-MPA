...
@section('content')
    <div class="card">
        <div class="card-header">
            ...
            <small>{{$item->code_transaction}}</small>
        </div>
        <div class="card-body card-block">
            <form action="{{ route('transactions.update', $item->id) }}" method="post">
                ...
                <div class="form-group">
                    ...
                    <input type="text"
                            name="cust_name"
                            value="{{ old('cust_name') ? old('cust_name') : $item->cust_name }}"
                            class="form-control @error('cust_name') is-invalid @enderror"/>
                @error('cust_name') <div class="text-muted">{{$message}}</div> @enderror
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text"
                            name="number"
                            value="{{ old('number') ? old('number') : $item->number }}" {{-- tenary function/pengkondisian --}}
